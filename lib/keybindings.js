 ...
 * @returns {{ code: string, key?: string }|null} `key` is filled in for letters. Null for a key that
 *   cannot be placed.
 */
 ...
 *
 * @param {string} text
 * @returns {import('../types').Step|null} Null for a key that cannot be placed.
 */
...
function resolveChords({ defaults, user, commands }) {
  ...
  for (const entry of user) {
    if (typeof entry.command === 'string' && entry.command.startsWith('-')) {
      ...
      if (!chord) continue; // a removal that cannot be read must remove nothing, not everything
