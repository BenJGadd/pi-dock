 ...
 * token that only our page is given. There is no `-O`, ttyd's check that the page came from ttyd
 ...
 * terminal would eventually break something.
 *
 * @param {NodeJS.ProcessEnv} env Normally process.env.
 * @returns {NodeJS.ProcessEnv} A copy with COLORTERM set.
 */
function ttydEnv(env) {
...
module.exports = { ttydEnv, buildArgs, freePort, waitForPort, tailLines };
